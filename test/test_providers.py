...
class GenericProviderTest:
    ...
    def testProvisionReleaseNoStartEndTime(self):
        ...
        criteria = nsa.Criteria(0, schedule,
                                nsa.Point2PointService(self.source_stp, self.dest_stp, 200, 'Bidirectional', False,
                                                       None))
    ...
    def testInvalidNetworkReservation(self):
        ...
        criteria = nsa.Criteria(0, self.schedule,
                                nsa.Point2PointService(source_stp, dest_stp, 200, 'Bidirectional', False, None))
    ...
    def testLabelRangeMultiReservation(self):
        ...
        criteria = nsa.Criteria(0, self.schedule,
                                nsa.Point2PointService(source_stp, dest_stp, 100, 'Bidirectional', False, None))
    ...
    def testReserveAbort(self):
        ...
        criteria = nsa.Criteria(0, self.schedule,
                                nsa.Point2PointService(source_stp, dest_stp, 200, cnt.BIDIRECTIONAL, False, None))
    ...
    def testNoEndtimeAbort(self):
        ...
        dest_stp = nsa.STP(self.network, self.dest_port, nsa.Label(cnt.ETHERNET_VLAN, '1782'))
        end_time = None
        criteria = nsa.Criteria(0, nsa.Schedule(self.start_time, end_time),
                                nsa.Point2PointService(source_stp, dest_stp, 200, cnt.BIDIRECTIONAL, False, None))
    ...
    def testReserveFailAndLabelSwapEnabled(self):
        ...
        self.assertTrue(self.backend.connection_manager.canSwapLabel(cnt.ETHERNET_VLAN),
                        "DUD is not able to swapLabels")
        ...
        criteria2 = nsa.Criteria(0, self.schedule,
                                 nsa.Point2PointService(source_stp2, dest_stp, 200, cnt.BIDIRECTIONAL, False, None))
        ...
        for stp in [source_stp2, dest_stp, source_stp]:
            try:
                ...
                resource_is_available = self.backend.calendar.checkReservation(res, self.schedule.start_time,
    ...
    def testReserveTimeout(self):
        ...
        dest_stp = nsa.STP(self.network, self.dest_port, nsa.Label(cnt.ETHERNET_VLAN, '1782'))
        criteria = nsa.Criteria(0, self.schedule,
                                nsa.Point2PointService(source_stp, dest_stp, self.bandwidth, cnt.BIDIRECTIONAL, False,
                                                       None))
    ...
    def testFaultyActivate(self):
        ...
        self.backend.connection_manager.setupLink = lambda cid, src, dst, bw: defer.fail(
            error.InternalNRMError('Link setup failed'))
    ...
    def testFaultyDeactivate(self):
        ...
        # make actication fail via monkey patching
        self.backend.connection_manager.teardownLink = lambda cid, src, dst, bw: defer.fail(
            error.InternalNRMError('Link teardown failed'))
    ...
    def testInvalidRewrite(self):
        ...
        criteria = nsa.Criteria(0, self.schedule,
                                nsa.Point2PointService(source_stp, self.dest_stp, 200, cnt.BIDIRECTIONAL, False, None))
    ...
    def testNoStartEndTimeAndAdditionalReservation(self):
        ...
        schedule = nsa.Schedule(None, None)
        criteria = nsa.Criteria(0, schedule,
                                nsa.Point2PointService(self.source_stp, self.dest_stp, 200, cnt.BIDIRECTIONAL, False,
                                                       None))
...
class RemoteProviderTest(GenericProviderTest, unittest.TestCase):
    ...
    requester_agent = nsa.NetworkServiceAgent('test-requester:nsa',
                                              'http://localhost:%i/NSI/services/RequesterService2' % REQUESTER_PORT)
    provider_agent = nsa.NetworkServiceAgent(GenericProviderTest.base + ':nsa',
                                             'http://localhost:%i/NSI/services/CS2' % PROVIDER_PORT)
    ...
    def setUp(self):
        ...
        pr = provreg.ProviderRegistry({})
        pr.addProvider(self.provider_agent.urn(), self.network, self.backend)
        self.aggregator = aggregator.Aggregator(self.provider_agent, nml_network, link_vector, self.requester, pr, [],
                                                pl)
        ...
        requester_service = requesterservice.RequesterService(soap_resource,
                                                              self.requester)  # this is the important part
