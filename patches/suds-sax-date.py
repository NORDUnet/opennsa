...
class Time:
    ...
    def __parse(self, s):
        ...
        try:
            ...
            if len(part) == 2:
                ...
                tz = Timezone(offset)
            if ms is None:
                return dt.time(hour, minute, second, tzinfo=tz)
            else:
                return dt.time(hour, minute, second, ms, tzinfo=tz)
        except:
            log.debug(s, exec_info=True)
            raise ValueError, 'Invalid format "%s"' % s
    ...
    def __offset(self, s):
        ...
        @rtype: str
        """
        if len(s) == 0: # no time zone specified
            return None
        if s == 'Z':
            return dt.timedelta()
        if len(s) == len('-00:00'):
            ...
            if prefix == '-':
            	...
            	return dt.timedelta(seconds=seconds)
        raise ValueError('Cannot parse timezone offset %s' % s)
...
class Timezone(dt.tzinfo):
    ...
    def __init__(self, offset):
        """
        Create the timezone.
        @param s: A timedelta representation of the TZ offset.
        @type s: datetime.timedelta
        """
        self.offset = offset
