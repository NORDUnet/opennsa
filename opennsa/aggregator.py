...
def _createAggregateException(connection_id, action, results, provider_urns, default_error=error.InternalServerError):
    ...
    if len(failures) == 1:
        ...
        provider_failures = [provider_urn + ': ' + f.getErrorMessage() for provider_urn, (success, f) in
                             zip(provider_urns, results) if not success]
...
class Aggregator:
    ...
    def getSubConnection(self, provider_nsa, connection_id):
        ...
        def gotResult(connections):
            ...
            if len(connections) == 0:
                return defer.fail(error.ConnectionNonExistentError(
                    'No sub connection with connection id %s at provider %s' % (connection_id, provider_nsa)))
    ...
    def getSubConnectionsByConnectionKey(self, service_connection_key):
        ...
        d = dbconfig.select('sub_connections', where=['service_connection_id = ?', service_connection_key],
                            select='provider_nsa, connection_id')
        d.addCallback(gotResult)
        return d
    ...
    def reserve(self, header, connection_id, global_reservation_id, description, criteria, request_info=None):
        ...
        log.msg('- Path %s -- %s ' % (criteria.service_def.source_stp, criteria.service_def.dest_stp),
                system=LOG_SYSTEM)
        ...
        if cnt.REQUIRE_TRACE in self.policies:
            if not header.connection_trace:
                ...
                raise error.SecurityError(
                    'This NSA (%s) requires a connection trace in the header to create a reservation.' % self.nsa_.urn())
        ...
        if cnt.REQUIRE_USER in self.policies:
            ...
            if not user_attrs:
                ...
                raise error.SecurityError(
                    'This NSA (%s) requires a user attribute in the header to create a reservation.' % self.nsa_.urn())
        ...
        if not cnt.AGGREGATOR in self.policies:
            ...
            if not (source_stp.network in local_networks or dest_stp.network in local_networks):
                raise error.ConnectionCreateError(
                    'None of the endpoints terminate in the network, rejecting request (network: %s + %s, local networks %s)' %
        ...
        if any([source_stp.network in local_networks, dest_stp.network in local_networks]):
            if source_stp.network not in local_networks and self.route_vectors.vector(source_stp.network,
                                                                                      source=source_stp.network) is None:
                raise error.ConnectionCreateError('No known routes to network %s' % source_stp.network)
            if dest_stp.network not in local_networks and self.route_vectors.vector(dest_stp.network,
                                                                                    source=source_stp.network) is None:
        ...
        conn = database.ServiceConnection(connection_id=connection_id, revision=0,
                                          global_reservation_id=global_reservation_id, description=description,
                                          requester_nsa=header.requester_nsa, requester_url=header.reply_to,
                                          ...
                                          source_label=source_stp.label,
                                          dest_network=dest_stp.network, dest_port=dest_stp.port,
                                          dest_label=dest_stp.label,
        ...
        if conn.source_network == conn.dest_network and conn.source_network in local_networks:  # and conn.dest_network in self.network:
            ...
            # setup path
            path_info = (
                conn.connection_id, conn.source_network, conn.source_port, shortLabel(conn.source_label),
                conn.dest_port,
                shortLabel(conn.dest_label))
            ...
            log.msg('Connection %s: Aggregate path creation: %s -> %s' % (
                conn.connection_id, str(source_stp), str(dest_stp)), system=LOG_SYSTEM)
            ...
            for v_network, v_port in setup_vector:
                assert prev_stp.network == v_network, 'network mismatch during cross connect building {} != {}'.format(
                    prev_stp.network, v_network)
                ...
                prev_stp = nsa.STP(vector_nrm_port.remote_network, vector_nrm_port.remote_port,
                                   vector_nrm_port.label)  # the is sorta from the wrong side, but they should be identical
            ...
            raise error.ConnectionCreateError(
                'None of the endpoints terminate in the network, rejecting request (network: %s + %s, nsa network %s)' %
        ...
        for link in selected_path:
            ...
            if p is None:
                raise error.ConnectionCreateError(
                    'No provider for network %s. Cannot create link.' % link.src_stp.network)
        ...
        for idx, link in enumerate(selected_path):
            ...
            c_header = nsa.NSIHeader(self.nsa_.urn(), provider_urn, security_attributes=header.security_attributes,
                                     connection_trace=conn_trace)
        ...
        if all(successes):
            ...
            defs = []
            reserved_connections = [(sc_id, network_urn) for (success, sc_id), (_, network_urn) in
                                    zip(results, conn_info) if success]
            for (sc_id, network_urn) in reserved_connections:
                ...
                d.addCallbacks(
                    lambda c: log.msg(
                        'Succesfully terminated sub connection %s at %s after partial reservation failure.' % (
                            sc_id, provider_urn), system=LOG_SYSTEM),
            ...
            err = _createAggregateException(connection_id, 'reservations', results, provider_urns,
    ...
    def reserveCommit(self, header, connection_id, request_info=None):
        ...
        log.msg('', system=LOG_SYSTEM)
        log.msg('ReserveCommit request. NSA: %s. Connection ID: %s' % (header.requester_nsa, connection_id),
                system=LOG_SYSTEM)
        ...
        for sc in sub_connections:
            ...
            provider = self.provider_registry.getProvider(
                sc.source_network)  # source and dest network should be the same
        ...
        if all(successes):
            ...
            log.msg('Connection %s. Only %i of %i commit acked successfully' % (connection_id, n_success, len(defs)),
                    system=LOG_SYSTEM)
    ...
    def reserveAbort(self, header, connection_id, request_info=None):
        ...
        log.msg('', system=LOG_SYSTEM)
        log.msg('ReserveAbort request. NSA: %s. Connection ID: %s' % (header.requester_nsa, connection_id),
                system=LOG_SYSTEM)
        ...
        if all(successes):
            ...
            n_success = sum([1 for s in successes if s])
            log.msg(
                'Connection %s. Only %i of %i connections aborted' % (conn.connection_id, len(n_success), len(defs)),
                system=LOG_SYSTEM)
            provider_urns = [sc.provider_nsa for sc in sub_connections]
    ...
    def provision(self, header, connection_id, request_info=None):
        ...
        log.msg('', system=LOG_SYSTEM)
        log.msg('Provision request. NSA: %s. Connection ID: %s' % (header.requester_nsa, connection_id),
                system=LOG_SYSTEM)
        ...
        if all(successes):
            ...
            n_success = sum([1 for s in successes if s])
            log.msg('Connection %s. Provision failure. %i of %i connections successfully acked' % (
                connection_id, n_success, len(defs)), system=LOG_SYSTEM)
            provider_urns = [sc.provider_nsa for sc in sub_connections]
    ...
    def release(self, header, connection_id, request_info=None):
        ...
        log.msg('', system=LOG_SYSTEM)
        log.msg('Release request. NSA: %s. Connection ID: %s' % (header.requester_nsa, connection_id),
                system=LOG_SYSTEM)
        ...
        if all(successes):
            ...
            n_success = sum([1 for s in successes if s])
            log.msg('Connection %s. Only %i of %i connections successfully released' % (
    ...
    def terminate(self, header, connection_id, request_info=None):
        ...
        log.msg('', system=LOG_SYSTEM)
        log.msg('Terminate request. NSA: %s. Connection ID: %s' % (header.requester_nsa, connection_id),
                system=LOG_SYSTEM)
        ...
        successes = [r[0] for r in results]
        if all(successes):
            log.msg('Connection %s: All sub connections(%i) acked terminated' % (conn.connection_id, len(defs)),
                    system=LOG_SYSTEM)
            defer.returnValue(connection_id)
        else:
            # we are now in an inconsistent state...
            n_success = sum([1 for s in successes if s])
            log.msg('Connection %s. Only %i of %i connections successfully terminated' % (
                conn.connection_id, n_success, len(defs)), system=LOG_SYSTEM)
            provider_urns = [sc.provider_nsa for sc in sub_connections]
    ...
    def querySummary(self, header, connection_ids=None, global_reservation_ids=None, request_info=None):
        ...
        log.msg('QuerySummary request from %s. CID: %s. GID: %s' % (
            header.requester_nsa, connection_ids, global_reservation_ids), system=LOG_SYSTEM)
        admin_override = Config.instance().is_admin_override(header.requester_nsa)
        parameters = []
        query = 'true'

        if not admin_override:
            query = 'requester_nsa = ?'
            parameters = [header.requester_nsa]

        try:
            if connection_ids:
                parameters.append(tuple(connection_ids))
                conns = yield database.ServiceConnection.find(
                    where=[f'{query} AND connection_id IN ?', *parameters])
            elif global_reservation_ids:
                parameters.append(tuple(global_reservation_ids))
                conns = yield database.ServiceConnection.find(
                    where=[f'{query} AND global_reservation_ids IN ?', *parameters])
            else:
                if len(parameters) == 0:
                    conns = yield database.ServiceConnection.find()
                else:
                    conns = yield database.ServiceConnection.find(where=[f'{query}', *parameters])
            ...
            for c in conns:
                ...
                ci = nsa.ConnectionInfo(c.connection_id, c.global_reservation_id, c.description, cnt.EVTS_AGOLE,
                                        [criteria],
    ...
    def queryRecursive(self, header, connection_ids, global_reservation_ids, request_info=None):
        ...
        log.msg('QueryRecursive request from %s. CID: %s. GID: %s' % (
            header.requester_nsa, connection_ids, global_reservation_ids), system=LOG_SYSTEM)
        ...
        if global_reservation_ids:
            raise error.UnsupportedParameter(
                "Global Reservation Id not supported in queryRecursive (has wonky-monkey-on-acid semantics, don't use it)")

        # recursive queries for all connections is a bad idea, say no to that
        if not connection_ids:
            raise error.MissingParameterError(
                "At least one connection id must be specified, refusing to do recursive query for all connections")
        ...
        try:
            ...
            cb_header = nsa.NSIHeader(header.requester_nsa, self.nsa_.urn(), header.correlation_id,
                                      reply_to=header.reply_to, security_attributes=header.security_attributes)
            ...
            if all(successes):
                ...
                n_success = sum([1 for s in successes if s])
                log.msg('QueryRecursive failure. %i of %i connections successfully replied' % (n_success, len(defs)),
                        system=LOG_SYSTEM)
    ...
    def queryRecursiveConfirmed(self, header, sub_result):
        ...
        if not header.correlation_id in self.query_calls:
            log.msg('queryRecursiveConfirmed could not match correlation id %s' % header.correlation_id,
                    system=LOG_SYSTEM)
            return

        cbh_correlation_id, res = self.query_calls[header.correlation_id]
        if res:
            log.msg('queryRecursiveConfirmed : Already have result for correlation id %s' % header.correlation_id,
                    system=LOG_SYSTEM)
            return
        ...
        if len(scr) == count:
            ...
            log.msg('QueryRecursive : Still neeed %i/%i results to emit result' % (count - len(scr), count),
                    system=LOG_SYSTEM)

    def queryNotification(self, header, connection_id, start_notification, end_notification):

        log.msg('QueryNotification request from %s. CID: %s. %s-%s' % (
            header.requester_nsa, connection_id, start_notification, end_notification), system=LOG_SYSTEM)
    ...
    def reserveConfirmed(self, header, connection_id, global_reservation_id, description, criteria):
        ...
        if not header.correlation_id in self.reservations:
            msg = 'Unrecognized correlation id %s in reserveConfirmed. Connection ID %s. NSA %s' % (
                header.correlation_id, connection_id, header.provider_nsa)
        ...
        if header.provider_nsa != org_provider_nsa:
            log.msg('Provider NSA in header %s for reserveConfirmed does not match saved identity %s' % (
                header.provider_nsa, org_provider_nsa), system=LOG_SYSTEM)
        ...
        if sd.source_stp.network != resv_info['source_network']:
            log.msg('reserveConfirmed: source network mismatch (%s != %s)' % (
                resv_info['source_network'], sd.source_stp.network), system=LOG_SYSTEM)
        if sd.source_stp.port != resv_info['source_port']:
            log.msg('reserveConfirmed: source port mismatch (%s != %s' % (resv_info['source_port'], sd.source_stp.port),
                    system=LOG_SYSTEM)
        ...
        sc = database.SubConnection(provider_nsa=org_provider_nsa, connection_id=connection_id, local_link=False,
                                    # remove local link sometime
                                    revision=criteria.revision,
                                    service_connection_id=resv_info['service_connection_id'],
                                    order_id=resv_info['order_id'],
                                    global_reservation_id=global_reservation_id, description=description,
                                    reservation_state=state.RESERVE_HELD, provision_state=state.RELEASED,
                                    lifecycle_state=state.CREATED, data_plane_active=False,
                                    source_network=sd.source_stp.network, source_port=sd.source_stp.port,
                                    source_label=sd.source_stp.label,
                                    dest_network=sd.dest_stp.network, dest_port=sd.dest_stp.port,
                                    dest_label=sd.dest_stp.label,
        ...
        outstanding_calls = [v for v in self.reservations.values() if
                             v.get('service_connection_id') == resv_info['service_connection_id']]
        if len(outstanding_calls) > 0:
            log.msg('Connection %s: Still missing %i reserveConfirmed call(s) to aggregate' % (
                conn.connection_id, len(outstanding_calls)), system=LOG_SYSTEM)
            return

        # if we get responses very close, multiple requests can trigger this, so we check main state as well
        if all([sc.reservation_state == state.RESERVE_HELD for sc in
                sub_conns]) and conn.reservation_state != state.RESERVE_HELD:
            log.msg('Connection %s: All sub connections reserve held, can emit reserveConfirmed' % (conn.connection_id),
                    system=LOG_SYSTEM)
            ...
            sd = nsa.Point2PointService(source_stp, dest_stp, conn.bandwidth, cnt.BIDIRECTIONAL, False,
                                        None)  # we fake some thing that is not yet in the db
            ...
            self.parent_requester.reserveConfirmed(header, conn.connection_id, conn.global_reservation_id,
                                                   conn.description, conn_criteria)

        else:
            log.msg('Connection %s: Still missing reserveConfirmed messages before emitting to parent' % (
                conn.connection_id), system=LOG_SYSTEM)

    @defer.inlineCallbacks
    def reserveFailed(self, header, connection_id, connection_states, err):

        log.msg('', system=LOG_SYSTEM)
        log.msg('reserveFailed from %s. Connection ID: %s. Error: %s' % (header.provider_nsa, connection_id, err),
                system=LOG_SYSTEM)

        if not header.correlation_id in self.reservations:
            msg = 'Unrecognized correlation id %s in reserveFailed. Connection ID %s. NSA %s' % (
                header.correlation_id, connection_id, header.provider_nsa)
            log.msg(msg, system=LOG_SYSTEM)
            raise error.ConnectionNonExistentError(msg)
        ...
        org_provider_nsa = self.reservations[header.correlation_id]['provider_nsa']
        if header.provider_nsa != org_provider_nsa:
            log.msg('Provider NSA in header %s for reserveFailed does not match saved identity %s' % (
                header.provider_nsa, org_provider_nsa), system=LOG_SYSTEM)
            raise error.SecurityError('Provider NSA for connection does not match saved identity')
    ...
    def reserveCommitConfirmed(self, header, connection_id):
        ...
        log.msg('', system=LOG_SYSTEM)
        log.msg('ReserveCommit Confirmed for sub connection %s. NSA %s ' % (connection_id, header.provider_nsa),
                system=LOG_SYSTEM)
        ...
        # if we get responses very close, multiple requests can trigger this, so we check main state as well
        if all([sc.reservation_state == state.RESERVE_START for sc in
                sub_conns]) and conn.reservation_state != state.RESERVE_START:
    ...
    def reserveAbortConfirmed(self, header, connection_id):
        ...
        log.msg('', system=LOG_SYSTEM)
        log.msg('ReserveAbort confirmed for sub connection %s. NSA %s ' % (connection_id, header.provider_nsa),
                system=LOG_SYSTEM)
    ...
    def provisionConfirmed(self, header, connection_id):
        ...
        log.msg('', system=LOG_SYSTEM)
        log.msg('Provision Confirmed for sub connection %s. NSA %s ' % (connection_id, header.provider_nsa),
                system=LOG_SYSTEM)
        ...
        # if we get responses very close, multiple requests can trigger this, so we check main state as well
        if all([sc.provision_state == state.PROVISIONED for sc in
                sub_conns]) and conn.provision_state != state.PROVISIONED:
    ...
    def releaseConfirmed(self, header, connection_id):
        ...
        log.msg('', system=LOG_SYSTEM)
        log.msg('Release confirmed for sub connection %s. NSA %s ' % (connection_id, header.provider_nsa),
                system=LOG_SYSTEM)
    ...
    def terminateConfirmed(self, header, connection_id):
        ...
        # if we get responses very close, multiple requests can trigger this, so we check main state as well
        if all([sc.lifecycle_state == state.TERMINATED for sc in
                sub_conns]) and conn.lifecycle_state != state.TERMINATED:
            yield state.terminated(conn)
            header = nsa.NSIHeader(conn.requester_nsa, self.nsa_.urn())
    ...
    def doTimeout(self, conn, timeout_value, org_connection_id, org_nsa):
        ...
        self.parent_requester.reserveTimeout(header, conn.connection_id, 0, now, timeout_value, org_connection_id,
                                             org_nsa)
    ...
    @defer.inlineCallbacks
    def reserveTimeout(self, header, connection_id, notification_id, timestamp, timeout_value, org_connection_id,
                       org_nsa):
        ...
        if conn.reservation_state == state.RESERVE_FAILED:
            log.msg(
                "Connection %s: reserveTimeout: Connection has already failed, not notifying parent" % conn.connection_id,
                system=LOG_SYSTEM)
        elif sum([1 if sc.reservation_state == state.RESERVE_TIMEOUT else 0 for sc in sub_conns]) == 1:
            log.msg("Connection %s: reserveTimeout, first occurance, notifying parent" % conn.connection_id,
                    system=LOG_SYSTEM)
            header = nsa.NSIHeader(conn.requester_nsa, self.nsa_.urn(), reply_to=conn.requester_url)
            self.parent_requester.reserveTimeout(header, conn.connection_id, notification_id, timestamp, timeout_value,
                                                 org_connection_id, org_nsa)
        else:
            log.msg(
                "Connection %s: reserveTimeout: Second or later reserveTimeout, not notifying parent" % conn.connection_id,
                system=LOG_SYSTEM)
    ...
    def dataPlaneStateChange(self, header, connection_id, notification_id, timestamp, dps):
        ...
        aggr_consistent = all([sc.data_plane_consistent for sc in sub_conns]) and all(
            [a == actives[0] for a in actives])  # we need version here
    ...
    def error(self, header, nsa_id, connection_id, service_type, error_id, text, variables, child_ex):
        ...
        log.msg("errorEvent: Connection %s from %s: %s, %s" % (connection_id, nsa_id, text, str(variables)),
                system=LOG_SYSTEM)

        if header.provider_nsa != nsa_id:
            log.msg(
                "errorEvent: NSA Id for error is different from provider (provider: %s, nsa: %s, cannot handle error, due to protocol design issue." % \
