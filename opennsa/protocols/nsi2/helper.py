...
def export(type_binding, namespace, name):

    f = StringIO.StringIO()
    type_binding.export(f, 0, namespacedef_='xmlns:tns="%s"' % namespace, name_=name)
    return f.getvalue()
...
def createHeader(correlation_id, requester_nsa_urn, provider_nsa_urn, reply_to=None):
    ...
    header_payload = export(header, FRAMEWORK_TYPES_NS, 'nsiHeader')
